...
- **00-AC-2.** THE SYSTEM SHALL declare three targets: `PastaEngine` (framework),
  `PastaApp` (application, depends on `PastaEngine`), and `PastaEngineTests` (unit-test
  bundle for `PastaEngine`).
- **00-AC-3.** THE SYSTEM SHALL set the deployment target to macOS 13.0 for all targets.
- **00-AC-4.** THE SYSTEM SHALL configure `PastaApp` as an agent app via
...
- **00-AC-5.** THE SYSTEM SHALL declare the `KeyboardShortcuts` SPM dependency
  (`https://github.com/sindresorhus/KeyboardShortcuts`, from 2.0.0) on `PastaApp` and
  commit a pinned `Package.resolved`.
- **00-AC-6.** THE SYSTEM SHALL reference an entitlements file
  (`Sources/PastaApp/Resources/pasta.entitlements`); the MVP build SHALL be
...
- **00-AC-8.** WHEN `make gen && make build` runs on a Mac with XcodeGen installed,
  THE SYSTEM SHALL produce a `PastaApp.app` bundle that launches and shows no Dock icon.
